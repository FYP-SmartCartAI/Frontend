...
// Role-scoped admin transition rules
...
export default function AdminOrdersPage() {
  ...
  const { data, isLoading }     = useGetAllOrdersQuery({ page, limit: 15, status: statusFilter || undefined })
        ...
        <div className="flex gap-1.5 overflow-x-auto">
          {['', 'pending', 'processing', 'shipped', 'delivered', 'cancelled'].map((s) => (
            <button
              ...
              className={cn(
                'px-3 py-1 text-xs rounded-full border whitespace-nowrap shrink-0 transition-all',
                statusFilter === s
            ...
            <tbody>
              {isLoading
                ...
                : orders.map((order) => {
                    ...
                    const allowedNext = (ADMIN_TRANSITIONS[currentStatus] || [])
                    ...
                    return (
                      <tr key={order._id} className="border-b border-border last:border-0 hover:bg-bg-tertiary transition-colors">
                        <td className="px-4 py-3 font-[var(--font-mono)] text-text-secondary text-xs">
